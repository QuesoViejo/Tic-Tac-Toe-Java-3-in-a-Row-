...
public class Ventana1 extends JFrame {
	...
	int[] vec = {0,0,0,0,0,0,0,0,0}; // vec ya está inicializado
	int turno = 1;
	...
	public Ventana1() {
		...
		textField.setText("Turno Player 1");
		...
		btt1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent example1) {
				...
				if(turno%2 == 0&& turno < 9) {
					textField.setText("Turno Player 2");
				}else {
					if(turno >= 9) {
						Comprobacion(vec);
					}
	...
	private boolean Comprobacion(int vec[]){
		...
		int winner = 0; // Lo inicializamos en 0 que es por defecto el valor con el que se sigue jugando
		//Comprobaciones como en C. Si se encuentra: ganador = 0(todavia nada) 1(player1) 2(player2) 3(empate)
		...
		if(winner != 0) {
			...
			//Voy a dejar todos los botones en disabled y listo, no sé usar un sleep
